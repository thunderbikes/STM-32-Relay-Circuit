...
void MX_TIM2_Init(void) {
    ...
    htim2.Instance = TIM2;
    htim2.Init.Prescaler = 4799;  // Reduces the timer clock to 10kHz
    htim2.Init.CounterMode = TIM_COUNTERMODE_UP;
    htim2.Init.Period = 5000 - 1;  // 500 ms period at 10 kHz frequency
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim){
  ...
  switch(Status) {
    ...
    case PRECHARGE:
      updating_counter = 0;
      if(HAL_GPIO_ReadPin(GPIOB, R1_AUX_GPIO_Port) == GPIO_PIN_RESET){ //please double check all AUX pins -ray
        if(HAL_GPIO_ReadPin(GPIOB, R2_AUX_GPIO_Port) == GPIO_PIN_SET){ //also 3 if statements seems a bit more readable
          if(HAL_GPIO_ReadPin(GPIOB, R3_AUX_GPIO_Port) == GPIO_PIN_SET){
            break;
          }
        }
      }
    ...
    case OPERATION:
      updating_counter = 0;
      if(HAL_GPIO_ReadPin(GPIOB, R1_AUX_GPIO_Port) == GPIO_PIN_SET){
        if(HAL_GPIO_ReadPin(GPIOB, R2_AUX_GPIO_Port) == GPIO_PIN_SET){
          if(HAL_GPIO_ReadPin(GPIOB, R3_AUX_GPIO_Port) == GPIO_PIN_RESET){
            break;
          }
        }
      }
    ...
    case DISCHARGE:
      updating_counter = 0;
      if(HAL_GPIO_ReadPin(GPIOB, R1_AUX_GPIO_Port) == GPIO_PIN_RESET){
        if(HAL_GPIO_ReadPin(GPIOB, R2_AUX_GPIO_Port) == GPIO_PIN_RESET){
          if(HAL_GPIO_ReadPin(GPIOB, R3_AUX_GPIO_Port) == GPIO_PIN_RESET){
            break;
          }
        }
