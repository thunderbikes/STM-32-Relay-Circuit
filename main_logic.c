...
int park(int forward_input, int reverse_input) {
    return (forward_input == 0 && reverse_input == 0) ? 1 : 0;
}
  ...
  while (1)
   {
    ...
    /* USER CODE BEGIN 3 */
         ...
         int park_input = park(Reading_Pin('f'), Reading_Pin('r'));
         ...
         */
         ...
         else
         {
             ...
             // CHARGING
             if (Reading_Pin('c') == 1 && park(Reading_Pin('f'), Reading_Pin('r')) == 1)
             {
                 allRelaysOpen();
                 ...
                 HAL_Delay(1000);
                 while (Reading_Pin('c') == 1)
                 {
                	 allDigitalRead();
                 	if (charging_aux() == 0){
                 		printDebug("Auxiliary Error during Charging");
                 		exit(0);
                 	} else {
                 		allDigitalRead();
                 		printDebug("Charging");
             ...
             // IGNITION
             else if (Reading_Pin('m') == 1 && park_input == 1 && operation == 0)
             {
                 allRelaysOpen();
                 ...
                 if (ignition_aux() == 0){
                 	printDebug("Auxiliary Error during Ignition");
                 	exit(0);
                 } else {
             ...
             // OPERATION
             else if (Reading_Pin('m') == 1 && operation == 1 && park(Reading_Pin('f'), Reading_Pin('r')) == 0)
             {
                 allRelaysOpen();
                 HAL_GPIO_WritePin(RL1_GPIO_Port, RL1_Pin, GPIO_PIN_SET);
                 HAL_Delay(1000);
                 HAL_GPIO_WritePin(RL2_GPIO_Port, RL2_Pin, GPIO_PIN_SET);
                 HAL_Delay(1000);// delay before aux
                 while (Reading_Pin('m') == 1 && operation == 1 && park(Reading_Pin('f'), Reading_Pin('r')) == 0)
                 {
                	allDigitalRead();
                 	if (operation_aux() == 0){
                 		printDebug("Auxiliary Error during Operation");
                 		exit(0);
                 	} else {
                     allDigitalRead();
                 	}
             ...
             // DISCHARGE
             else if (Reading_Pin('m') == 0 && park(Reading_Pin('f'), Reading_Pin('r')) == 1 && Reading_Pin('c') == 0 && operation == 1)
             {
                 allRelaysOpen();
                 HAL_GPIO_WritePin(RL4_GPIO_Port, RL4_Pin, GPIO_PIN_SET);
                 HAL_Delay(1000);// delay before aux
                 if (charging_aux() == 0){
                 	printDebug("Auxiliary Error during Discharge");
                 	exit(0);
                 } else {
                     HAL_Delay(5000);
                     allRelaysOpen();
             ...
             // EDGE CASES
             else if (Reading_Pin('m') ==1 && park(Reading_Pin('f'), Reading_Pin('r')) == 1)
             {
                 int main_state = Reading_Pin('m');
                 char str[2];
                 sprintf(str, "%d", main_state);
                 printDebug(str);
                 while (Reading_Pin('m') ==1 && park(Reading_Pin('f'),Reading_Pin('r')) == 1)
                 {
                     allDigitalRead();
                     HAL_Delay(1000);
                     allDigitalRead();
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  GPIO_InitStruct.Pin = BMS_Input2_Pin|BMS_Input1_Pin |R1_Aux_Pin|R3_Aux_Pin|R2_Aux_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
